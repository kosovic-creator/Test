...
export default async function Page() {
  ...
  return (
    ...
    <table className="flex flex-col items-center justify-center min-h-screen">
      ...
      <Link className='text-emerald-900' href="/test/new">Dodaj</Link>
      {foods.map(food => (
        <Link key={food.id} className='text-emerald-600' href={`/test/${food.id}/izmjeni`}>Prikaži</Link>
      ))}
      {foods.map(food => (
        <Link key={food.id} className='text-amber-700' href={`/test/${food.id}/delete`}>Ukloni</Link>
      ))}
      <ul>
        {foods.map(food => (
